...
class scene{
    ...
    std::vector<sphere> spheres;
        ...
        bool intersect(Ray& d, vector& pos, vector& n, int& sphere_id, double& t_min){
...
int main() {
    ...
    sphere s2(vector (0,-2000-20,0), 2000, vector(0,0,1)); // sol bleu
    sphere s3(vector (0,2000+100,0), 2000, vector(1,0,0)); // plafond rouge
    sphere s4(vector (-2000-50,0,0), 2000, vector(0,1,0)); // droite verte
    sphere s5(vector (2000+50,0,0), 2000, vector(1,0,1)); // gauche violette
	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
            ...
            if (has_intersection){
                ...
                Ray ray_light(pos + 0.01*n,(pos_light-pos).getnormalized());
                vector P_light, N_light;
                int sphere_id_light;
                double t_light;
                bool has_intersection_light = s.intersect(ray_light, P_light, N_light, sphere_id_light,t_light);
                double d_light2 = (pos_light - pos).norm2();

                if (has_intersection_light && t_light*t_light < d_light2){
                    intensity_pix = 0;
                } else {
                intensity_pix = intensity_light * max(dot((pos_light - pos).getnormalized(),n) ,0)/ d_light2;
                };
