...
class sphere{
	public:
		sphere(const vector& O, double R, const vector& color, const bool& miroir = false) : O(O), R(R), albedo(color), isMirror(miroir) {};
        vector O;
        double R;
        vector albedo;
        bool isMirror;
...
int main() {
    ...
    sphere s1(vector (-15,0,0) ,10 , vector (1,1,1), true); // sphere a gauche de la figure
    sphere s1_bis(vector (15,0,-30) ,10 , vector (1,1,1), true); // sphere a droite de la figure
    sphere s2(vector (0,-2000-30,0), 2000, vector(0,0,0.5)); // sol bleu
    sphere s3(vector (0,2000+100,0), 2000, vector(0.55,0,0)); // plafond rouge
    sphere s4(vector (-2000-80,0,30), 2000, vector(1,0.65,0)); // gauche jaune
    sphere s5(vector (2000+80,0,0), 2000, vector(0.28,0.24,0.80)); // droite violette
    ...
    s.add_sphere(s4);
    s.add_sphere(s5);
    s.pos_light = vector (-10,20,40); // position de la lumière
